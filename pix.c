...
char *fn(char *restrict, const char *restrict, const char *restrict);
...
int main(int argc, char **argv)
{
	...
	char *buffer = (char *)malloc(BUFSIZE * 50 * sizeof(char));
	char *buffer_ = (char *)malloc(BUFSIZE * 50 * sizeof(char));
	...
	memset(buffer, '\0', BUFSIZE * 50 * sizeof(char));
...
char *fn(char *restrict buffer, const char *restrict msg, const char *restrict addr)
{
	int sockfd = -1;
	...
	while ((recvb = read(sockfd, buffer + recvbyte, BUFSIZE * 50 - recvbyte)) > 0)
		recvbyte += recvb;
	close(sockfd);
	return buffer;
}
...
{
	char *buffer = (char *)malloc(BUFSIZE * 10 * sizeof(char));
	char ebuf[256] = {'\0'};
	regex_t reg;
	...
	memset(buffer, '\0', BUFSIZE * 10 * sizeof(char));
	...
	reg_err_code = regexec(&reg, string, nmatch, pmatch, 0);
	if (reg_err_code == REG_NOMATCH)
	{
